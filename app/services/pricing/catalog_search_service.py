...
class CatalogSearchService:
    ...
    def _enrich_with_pokemon_image(self, result: CatalogSearchResult) -> CatalogSearchResult:
        ...
        # 2026-08-29 (reviewer-approved): TCGdex (tcgdex_pokemon_catalog,
        # imported by scripts/import_tcgdex_pokemon_catalog.py) is now the
        # PRIMARY generic image source -- all English sets by normalized
        # set-name equality, "Pokemon Promo" rows routed to Black Star
        # Promo sets by number prefix, Japanese sets only through the
    ...
    def _fetch_tcgdex_pokemon_image(
        self, console_name: str, card_number: str
    ) -> str | None:
        # Deterministic set resolution only (see tcgdex_pokemon_sets.py):
        # unmapped Japanese sets and unroutable promo numbers mean no
        # attempt, never a fuzzy guess. Exactly one row must match --
        # zero is a miss, two+ is an ambiguity we don't trust.
        lowered = console_name.lower()
        if lowered.startswith("pokemon japanese"):
            ja_set_name = resolve_japanese_set_name(console_name)
            if ja_set_name is None:
                return None
            set_filter = {"language": "eq.ja", "set_name": f"eq.{ja_set_name}"}
        elif lowered == "pokemon promo":
            promo_set_key = resolve_promo_set_key(card_number)
            if promo_set_key is None:
                return None
            set_filter = {"language": "eq.en", "set_key": f"eq.{promo_set_key}"}
        else:
            set_key = resolve_english_set_key(console_name)
            if set_key is None:
                return None
            set_filter = {"language": "eq.en", "set_key": f"eq.{set_key}"}
        params = {
            "select": "image_url",
            **set_filter,
            "local_id_norm": f"eq.{normalize_card_number(card_number)}",
            "limit": "2",
        }
        payload = self._request("GET", "/rest/v1/tcgdex_pokemon_catalog", params=params)
        rows = (
            [row for row in payload if isinstance(row, dict)]
            if isinstance(payload, list)
            else []
        ...
        if len(rows) != 1:
            return None
        image_base = rows[0].get("image_url")
        if not image_base:
            return None
        # TCGdex asset base + documented quality/format suffix; 600x825,
        # the size their docs designate for "the main attraction" surface
...
_MAGIC_SET_ALIASES = {
    ...
    # Universes Beyond / renamed modern sets where PriceCharting's name
    # differs from Scryfall's. Every target below was verified present in
    # scryfall_magic_catalog (2026-08-30 whole-catalog audit: 82 PC sets
    # had no Scryfall match; these are the ones that resolve to a real
    # imported set -- the rest are ambiguous promo umbrellas / non-card
    # products / sets Scryfall's export doesn't carry, left unmatched).
    "marvel spider-man": "Marvel's Spider-Man",
    "marvel spider-man art series": "Marvel's Spider-Man Art Series",
    "marvel spider-man eternal": "Marvel's Spider-Man Eternal",
    ...
    "m11": "Magic 2011",
    "m10": "Magic 2010",
    "core set 2013": "Magic 2013",
    "core set 2012": "Magic 2012",
    "brother's war": "The Brothers' War",
    "brother's war commander": "The Brothers' War Commander",
    "brother's war retro artifacts": "The Brothers' War Retro Artifacts",
    "lost caverns of ixalan": "The Lost Caverns of Ixalan",
    "lost caverns of ixalan commander": "The Lost Caverns of Ixalan Commander",
    # Duel Decks: PriceCharting uses both "X vs Y" and "Duel Deck: X vs.
    # Y"; Scryfall models every one as "Duel Decks: X vs. Y".
    "izzet vs golgari": "Duel Decks: Izzet vs. Golgari",
    "duel deck: izzet vs. golgari": "Duel Decks: Izzet vs. Golgari",
    "ajani vs nicol bolas": "Duel Decks: Ajani vs. Nicol Bolas",
    "duel deck: ajani vs. nicol bolas": "Duel Decks: Ajani vs. Nicol Bolas",
    "speed vs cunning": "Duel Decks: Speed vs. Cunning",
    "duel deck: speed vs. cunning": "Duel Decks: Speed vs. Cunning",
    "knights vs dragons": "Duel Decks: Knights vs. Dragons",
    "duel deck: knights vs. dragons": "Duel Decks: Knights vs. Dragons",
    "jace vs vraska": "Duel Decks: Jace vs. Vraska",
    "duel deck: jace vs. vraska": "Duel Decks: Jace vs. Vraska",
    "nissa vs ob nixilis": "Duel Decks: Nissa vs. Ob Nixilis",
    "duel deck: nissa vs. ob nixilis": "Duel Decks: Nissa vs. Ob Nixilis",
    "elspeth vs tezzeret": "Duel Decks: Elspeth vs. Tezzeret",
    "duel deck: elspeth vs. tezzeret": "Duel Decks: Elspeth vs. Tezzeret",
    "venser vs koth": "Duel Decks: Venser vs. Koth",
    "duel deck: venser vs. koth": "Duel Decks: Venser vs. Koth",
    "phyrexia vs the coalition": "Duel Decks: Phyrexia vs. The Coalition",
    "duel deck: phyrexia vs. the coalition": "Duel Decks: Phyrexia vs. The Coalition",
    "heroes vs monsters": "Duel Decks: Heroes vs. Monsters",
    "duel deck: heroes vs. monsters": "Duel Decks: Heroes vs. Monsters",
    "zendikar vs eldrazi": "Duel Decks: Zendikar vs. Eldrazi",
    "duel deck: zendikar vs. eldrazi": "Duel Decks: Zendikar vs. Eldrazi",
    "sorin vs tibalt": "Duel Decks: Sorin vs. Tibalt",
    "duel deck: sorin vs. tibalt": "Duel Decks: Sorin vs. Tibalt",
    "elves vs goblins": "Duel Decks: Elves vs. Goblins",
    "duel deck: elves vs. goblins": "Duel Decks: Elves vs. Goblins",
    "garruk vs liliana": "Duel Decks: Garruk vs. Liliana",
    "duel deck: garruk vs. liliana": "Duel Decks: Garruk vs. Liliana",
    "divine vs demonic": "Duel Decks: Divine vs. Demonic",
    "duel deck: divine vs. demonic": "Duel Decks: Divine vs. Demonic",
    "jace vs chandra": "Duel Decks: Jace vs. Chandra",
    "duel deck: jace vs. chandra": "Duel Decks: Jace vs. Chandra",
    "blessed vs cursed": "Duel Decks: Blessed vs. Cursed",
    "duel deck: blessed vs. cursed": "Duel Decks: Blessed vs. Cursed",
    ...
    "duel deck: elves vs. inventors": "Duel Decks: Elves vs. Inventors",
