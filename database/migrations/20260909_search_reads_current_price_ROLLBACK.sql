-- Rollback for 20260909_search_reads_current_price.sql.
-- The exact definition of search_pricecharting_catalog as it stood in
-- production immediately before that migration was applied (2026-09-10).
-- Captured from pg_get_functiondef, not reconstructed from git, so it is
-- what was actually running rather than what should have been.
--
-- Applying this returns Discover search and browse to reading prices from
-- pricecharting_catalog. Safe while the catalog's cents are still being
-- maintained -- i.e. any time before PR 4.
